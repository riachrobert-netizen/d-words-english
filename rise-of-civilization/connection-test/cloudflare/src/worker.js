...
export default {
  async fetch(request, env) {
    ...
    const allowed = origin === 'https://riachrobert-netizen.github.io' || origin === url.origin || (
      url.hostname === 'localhost' && /^http:\/\/localhost:\d+$/.test(origin || '')
    );
...
export class Rooms extends DurableObject {
  ...
  send(ws, message) {
    ...
    if (ws.bufferedAmount > 65536) { ws.close(1013, 'Connection too slow'); return; }
  ...
  async webSocketMessage(ws, data) {
    ...
    if (typeof data !== 'string' || data.length > 4096) { this.error(ws, 'Invalid message.'); return; }
    ...
    if (m.type === 'create') {
      ...
      this.leave(ws); ws.serializeAttachment({ ...state, room: code }); this.update(code); return;
    }
    if (m.type === 'join') {
